...
audio = wave.open('chips_52.wav', 'wb')
...
while (cap.isOpened()):
    ...
    blue_upper = np.array([255, 200, 200], np.uint8)
    ...
    for pic, contour in enumerate(contours):
        ...
        if (area > 1300 and area < 1400):
            ...
            cv2.putText(img, "Spot {}{}".format(area, i), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0))

            M = cv2.moments(contour)  # Se obtiene el centro de masa de los marcadores enconrados.
            cx = (M['m10'] / M['m00'])
            cy = (M['m01'] / M['m00'])
            cv2.circle(img, (int(cx), int(cy)), 7, (255, 255, 255), -1)
            i = i + 1
            cy = cy - 379
